<?php
	...
	$GLOBALS['cfg']['mapzen_api_endpoint'] = '';
	$GLOBALS['cfg']['mapzen_oauth_endpoint'] = 'https://mapzen.com/oauth/';

	#################################################################

	function mapzen_oauth_get_request_token($args=array()){

		$keys = array(
			'oauth_key' => $GLOBALS['cfg']['mapzen_oauth_key'],
			'oauth_secret' => $GLOBALS['cfg']['mapzen_oauth_secret'],
		);

		$url = $GLOBALS['cfg']['mapzen_oauth_endpoint'] . 'request_token/';
		...
		$data = mapzen_oauth_rsp_to_hash($rsp['body']);
	...
	function mapzen_oauth_get_auth_url(&$args, &$user_keys){
		...
		$url = $GLOBALS['cfg']['mapzen_oauth_endpoint'] . 'authorize/';
		$url = oauth_sign_get($keys, $url, $args, 'GET');
	...
	function mapzen_oauth_get_access_token(&$args, &$user_keys){
		...
		$url = $GLOBALS['cfg']['mapzen_oauth_endpoint'] . 'token/';
	...
	function mapzen_oauth_api_call($method, $args, $more=array()){
		...
		$url = oauth_sign_get($keys, $GLOBALS['cfg']['mapzen_api_endpoint'], $args, 'POST');
		dumper($url);
	...
	function mapzen_oauth_rsp_to_hash($rsp){
